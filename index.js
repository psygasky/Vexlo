...
const KAMAN = process.env.KAMAN || '9afd1229-b893-40c1-84dd-51e7ce204913'; // 使用哪吒v1,在不同的平台运行需修改KAMAN,否则会覆盖
const YOUNGHERO_SERVER = process.env.YOUNGHERO_SERVER || '';        // 哪吒v1填写形式: nz.abc.com:8008  哪吒v0填写形式：nz.abc.com
const YOUNGHERO_PORT = process.env.YOUNGHERO_PORT || '';            // 使用哪吒v1请留空，哪吒v0需填写
const YOUNGHERO_KEY = process.env.YOUNGHERO_KEY || '';              // 哪吒v1的NZ_CLIENT_SECRET或哪吒v0的agent密钥
const SUIDAO_DOMAIN = process.env.SUIDAO_DOMAIN || '';          // 固定隧道域名,留空即启用临时隧道
const SUIDAO_AUTH = process.env.SUIDAO_AUTH || '';              // 固定隧道密钥json或token,留空即启用临时隧道,json获取地址：https://json.zone.id
const SUIDAO_PORT = process.env.SUIDAO_PORT || 8001;            // 固定隧道端口,使用token需在cloudflare后台设置和这里一致
const S5_PORT = process.env.S5_PORT || '';                  // socks5端口，支持多端口的可以填写，否则留空
const XIESIDILI_PORT = process.env.XIESIDILI_PORT || '';                // hy2端口，支持多端口的可以填写，否则留空
const XIANSHI_PORT = process.env.XIANSHI_PORT || '';        // reality端口，支持多端口的可以填写，否则留空
...
async function generateConfig() {
  const config = {
    ...
    inbounds: [
      { tag: 'vless-fallback-in', port: SUIDAO_PORT, listen: '::', protocol: 'vless', settings: { clients: [{ id: KAMAN, flow: 'xtls-rprx-vision' }], decryption: 'none', fallbacks: [{ dest: 3001 }, { path: "/vless-suidao", dest: 3002 }, { path: "/vmess-suidao", dest: 3003 }, { path: "/trojan-suidao", dest: 3004 }] }, streamSettings: { network: 'tcp' } },
      { tag: 'vless-tcp-in', port: 3001, listen: "127.0.0.1", protocol: "vless", settings: { clients: [{ id: KAMAN }], decryption: "none" }, streamSettings: { network: "tcp", security: "none" } },
      { tag: 'vless-ws-in', port: 3002, listen: "127.0.0.1", protocol: "vless", settings: { clients: [{ id: KAMAN, level: 0 }], decryption: "none" }, streamSettings: { network: "ws", security: "none", wsSettings: { path: "/vless-suidao" } }, sniffing: { enabled: true, destOverride: ["http", "tls", "quic"], metadataOnly: false } },
      { tag: 'vmess-ws-in', port: 3003, listen: "127.0.0.1", protocol: "vmess", settings: { clients: [{ id: KAMAN, alterId: 0 }] }, streamSettings: { network: "ws", wsSettings: { path: "/vmess-suidao" } }, sniffing: { enabled: true, destOverride: ["http", "tls", "quic"], metadataOnly: false } },
      { tag: 'trojan-ws-in', port: 3004, listen: "127.0.0.1", protocol: "trojan", settings: { clients: [{ password: KAMAN }] }, streamSettings: { network: "ws", security: "none", wsSettings: { path: "/trojan-suidao" } }, sniffing: { enabled: true, destOverride: ["http", "tls", "quic"], metadataOnly: false } },
    ],
  ...
  // VLESS Reality 配置
  if (isValidPort(XIANSHI_PORT)) {
    config.inbounds.push({
      tag: "vless-in",
      listen: "::",
      port: parseInt(XIANSHI_PORT),
      protocol: "vless",
      settings: {
        clients: [{ id: KAMAN, flow: "xtls-rprx-vision" }],
        decryption: "none"
      },
      streamSettings: {
        network: "raw",
  ...
  // Hysteria2 配置
  if (isValidPort(XIESIDILI_PORT)) {
    config.inbounds.push({
      tag: "hysteria-in",
      listen: "::",
      port: parseInt(XIESIDILI_PORT),
      protocol: "hysteria",
      settings: {
        version: 2,
        clients: [{ auth: KAMAN }]
...
function suidaoType() {
  if (!SUIDAO_AUTH || !SUIDAO_DOMAIN) {
    console.log("SUIDAO_DOMAIN or SUIDAO_AUTH is empty, use quick tunnels");
    return;
  }
  ...
  `;
    ...
    console.log(`Using token connect to tunnel, please set ${SUIDAO_PORT} in clouudflare`);
  }
}
...
async function generateLinks(suidaoDomain) {
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      const VMESS = { v: '2', ps: `${nodeName}`, add: CFIP, port: CFPORT, id: KAMAN, aid: '0', scy: 'auto', net: 'ws', type: 'none', host: suidaoDomain, path: '/vmess-suidao?ed=2560', tls: 'tls', sni: suidaoDomain, alpn: '', fp: 'firefox' };
      let subTxt = `
...
trojan://${KAMAN}@${CFIP}:${CFPORT}?security=tls&sni=${suidaoDomain}&fp=firefox&type=ws&host=${suidaoDomain}&path=%2Ftrojan-suidao%3Fed%3D2560#${nodeName}
    `;
      ...
      // XIESIDILI_PORT是有效端口号时生成hysteria2节点
      if (isValidPort(XIESIDILI_PORT)) {
        ...
        const hysteriaNode = `\nhysteria2://${KAMAN}@${SERVER_IP}:${XIESIDILI_PORT}/?sni=www.bing.com&insecure=0&alpn=h3&obfs=none${fingerprintParam}#${nodeName}`;
        subTxt += hysteriaNode;
      }

      // XIANSHI_PORT是有效端口号时生成reality节点
      if (isValidPort(XIANSHI_PORT)) {
        const vlessNode = `\nvless://${KAMAN}@${SERVER_IP}:${XIANSHI_PORT}?encryption=none&flow=xtls-rprx-vision&security=reality&sni=www.iij.ad.jp&fp=firefox&pbk=${publicKey}&type=tcp&headerType=none#${nodeName}`;
        subTxt += vlessNode;
      }

      // S5_PORT是有效端口号时生成socks5节点
      if (isValidPort(S5_PORT)) {
        const S5_AUTH = Buffer.from(`${KAMAN.substring(0, 8)}:${KAMAN.slice(-12)}`).toString('base64');
...
async function startserver() {
  try {
    ...
    // 生成 Reality 密钥对 (仅当 XIANSHI_PORT 开启才生成)
    if (isValidPort(XIANSHI_PORT)) {
